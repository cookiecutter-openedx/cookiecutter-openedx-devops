...
SET @source_db = 'edxapp';
SET @dest_db = 'openedx';

SELECT count(*) FROM @dest_db.badges_coursecompleteimageconfiguration;
SELECT count(*) FROM @dest_db.verify_student_manualverification;
...
INSERT INTO @dest_db.course_overviews_historicalcourseoverview (created, modified, version, id, _location, org, display_name, display_number_with_default, display_org_with_default, start, end , advertised_start, announcement, course_image_url, social_sharing_url, end_of_course_survey_url, certificates_display_behavior, certificates_show_before_end, cert_html_view_enabled, has_any_active_web_certificate, cert_name_short, cert_name_long, certificate_available_date, lowest_passing_grade, days_early_for_beta, mobile_available, visible_to_staff_only, _pre_requisite_courses_json, enrollment_start, enrollment_end, enrollment_domain, invitation_only, max_student_enrollments_allowed, catalog_visibility, short_description)
	...
	FROM	@dest_db.course_overviews_historicalcourseoverview i
			LEFT JOIN @dest_db.course_overviews_historicalcourseoverview d ON (i.history_id = d.history_id)
	WHERE	(d.history_id IS NULL);

INSERT INTO @dest_db.auth_user
	SELECT 	koa.*
	FROM 	@dest_db.auth_user koa
			LEFT JOIN @dest_db.auth_user maple ON (koa.username = maple.username)
	WHERE	(koa.id > 4) AND		-- the user records automatically created by Tutor
			(maple.id IS NULL);

INSERT INTO @dest_db.auth_userprofile (id, name, meta, courseware, language, location, year_of_birth, gender, level_of_education, mailing_address, city, country, goals, bio, profile_image_uploaded_at, user_id, phone_number, state)
    ...
    FROM @dest_db.auth_userprofile koa
		 JOIN @dest_db.auth_user koa_user ON (koa.user_id = koa_user.id)
		 JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
		 LEFT JOIN @dest_db.auth_userprofile maple ON (maple_user.id = maple.user_id)
	WHERE (maple.id IS NULL);

INSERT INTO @dest_db.student_loginfailures
	SELECT 	i.*
	FROM	@dest_db.student_loginfailures i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
			LEFT JOIN @dest_db.student_loginfailures d ON (maple_user.id = d.user_id)
	WHERE	(d.id IS NULL);

INSERT @dest_db.student_pendingemailchange
	SELECT	i.*
	FROM	@dest_db.student_pendingemailchange i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
            LEFT JOIN @dest_db.student_pendingemailchange d ON (maple_user.id = d.user_id)
	WHERE	(d.id IS NULL);

INSERT @dest_db.bookmarks_xblockcache
	SELECT 	i.*
	FROM	@dest_db.bookmarks_xblockcache i
			LEFT JOIN @dest_db.bookmarks_xblockcache d ON (i.course_key = d.course_key) AND (i.usage_key = d.usage_key)
	WHERE	(d.id IS NULL);

INSERT INTO @dest_db.bookmarks_bookmark (created, modified, course_key, usage_key, path, user_id, xblock_cache_id)
	SELECT	i.created, i.modified, i.course_key, i.usage_key, i.path, maple_user.id as user_id, i.xblock_cache_id
    FROM	@dest_db.bookmarks_bookmark i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
            LEFT JOIN @dest_db.bookmarks_bookmark dest ON ((i.course_key = dest.course_key) AND
														  (i.usage_key = dest.usage_key) AND
														  (dest.user_id = maple_user.id))
	WHERE 	(dest.id IS NULL);

INSERT INTO @dest_db.student_courseenrollment (course_id, created, is_active, mode, user_id)
	SELECT	i.course_id, i.created, i.is_active, i.mode, maple_user.id as user_id
	FROM 	@dest_db.student_courseenrollment i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
            LEFT JOIN @dest_db.student_courseenrollment d ON (d.user_id = maple_user.id)
	WHERE	(d.id IS NULL);

INSERT INTO @dest_db.student_courseenrollment_history
	...
	FROM	@dest_db.student_courseenrollment_history i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
            LEFT JOIN @dest_db.auth_user koa_history_user ON (i.history_user_id = koa_history_user.id)
            LEFT JOIN @dest_db.auth_user maple_history_user ON (koa_user.username = maple_history_user.username)
            LEFT JOIN @dest_db.student_courseenrollment_history d ON (i.id = d.id)
	WHERE	(d.id IS NULL);

INSERT INTO @dest_db.student_historicalmanualenrollmentaudit
	SELECT	i.*
	FROM	@dest_db.student_historicalmanualenrollmentaudit i
			LEFT JOIN @dest_db.student_historicalmanualenrollmentaudit d ON (i.id = d.id)
	WHERE	(d.id IS NULL);

INSERT INTO @dest_db.student_manualenrollmentaudit
	SELECT	i.*
	FROM	@dest_db.student_manualenrollmentaudit i
			LEFT JOIN @dest_db.student_manualenrollmentaudit d ON (i.id = d.id)
	WHERE	(d.id IS NULL);

INSERT INTO @dest_db.certificates_generatedcertificate
	...
	FROM	@dest_db.certificates_generatedcertificate i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
            LEFT JOIN @dest_db.certificates_generatedcertificate d ON (i.id = d.id)
	WHERE	(d.id IS NULL);

INSERT INTO @dest_db.certificates_historicalgeneratedcertificate (id, course_id, verify_uuid, download_uuid, download_url, grade, `key`, distinction, status, mode, name, created_date, modified_date, error_reason, history_id, history_date, history_type, history_user_id, user_id)
	...
	FROM	@dest_db.certificates_historicalgeneratedcertificate i
			JOIN @dest_db.auth_user koa_user ON (i.user_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
            LEFT JOIN @dest_db.auth_user koa_history_user ON (i.history_user_id = koa_history_user.id)
            LEFT JOIN @dest_db.auth_user maple_history_user ON (koa_user.username = maple_history_user.username)
            LEFT JOIN @dest_db.certificates_historicalgeneratedcertificate d ON (i.id = d.id)
	WHERE	(d.id IS NULL);
...
# ------------------------------------------------------------------- */
INSERT INTO @dest_db.courseware_studentmodule
	...
	FROM	@dest_db.courseware_studentmodule i
			JOIN @dest_db.auth_user koa_user ON (i.student_id = koa_user.id)
			JOIN @dest_db.auth_user maple_user ON (koa_user.username = maple_user.username)
			LEFT JOIN @dest_db.courseware_studentmodule d ON (i.id = d.id)
	WHERE	(d.id IS NULL)
