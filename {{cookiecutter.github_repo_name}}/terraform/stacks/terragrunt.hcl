locals {
  ...
  stack           = local.stacks_vars.locals.stack
  stack_namespace = local.stacks_vars.locals.stack_namespace
}
...
remote_state {
  ...
  config = {
    ...
    region         = local.aws_region
    dynamodb_table = "${local.stack_namespace}-terraform-lock"
    s3_bucket_tags = {
      Terraform       = "true"
